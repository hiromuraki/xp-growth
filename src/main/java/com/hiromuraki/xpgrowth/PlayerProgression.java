...
public final class PlayerProgression {
    ...
    private static void onLevelUp(ServerPlayer player, int previousLevel, int newLevel) {
        ...
        if (XPGrowthConfig.getMilestoneBonus()) {
            ...
            applyMilestoneBonus(player, tier);
        }

        if (XPGrowthConfig.getMilestoneFeedback()) {
            playMilestoneFeedback(player);
        }
    }

    /** 为玩家应用一次里程碑加成 */
    private static void applyMilestoneBonus(ServerPlayer player, int tier) {
    ...
    /** 为玩家播放一次里程碑效果 */
    private static void playMilestoneFeedback(ServerPlayer player) {
